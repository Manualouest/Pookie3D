...
t_p_vars cd_setup_p_vars(t_map_editor m_edit)
{
    ...
    while (++i < (m_edit.w_width - p_vars.left_gap * 2.0) + 1)
        p_vars.table_mx[i] = (int)floor((i) * (p_vars.nb_col / (m_edit.w_width - p_vars.left_gap * 2.0)));
    i = -1;
    while (++i < (m_edit.w_height - p_vars.up_gap * 2.0) + 1)
        p_vars.table_my[i] = (int)floor((i) * (p_vars.nb_line / (m_edit.w_height - p_vars.up_gap * 2.0)));
    return (p_vars);
}
