...
int	cd_add_texture(t_game *game, char *line)
{
	...
	if (line[0] < 32 || line[0] > 126 || !line[1] || line[1] != ' ')
	...
	if (game->graphic.paths[line[0] - 32] != NULL)
		free(game->graphic.paths[line[0] - 32]);
	game->graphic.paths[line[0] - 32] = n_line;
	game->graphic.slots[line[0] - 32] = 1;
	return (1);
}
